...
For each region and season-week, we averaged observations from the past four seasons (2015/2016 through 2018/2019) for the given season week and one season week on either side
...
```{r all-region-plot-ILI, fig.cap="\\label{fig:all-region-plot}US HHS Regions plots showing ILI- values since the 2015/2016 season (top), and rILI- values (bottom). The dates on the x-axis correspond to the dates for the 2019/2020 season, with previous seasons lined up approximately the same time by week. The vertical dashed line shows the date at which this plot was generated. The small gap between the current season's data and the line indicates the lag in ILI reporting, typically one week.", fig.height = 4.5}
...
p_ilineg2 <- ggplot(alldat2, aes(y=rili_minus, x=season_week, color=season)) + 
  geom_line() +
...
```{r std-dev-analysis, fig.height=3, fig.cap="Figure showing Z scores by week for each region."}
...
ggplot(filter(region_data, season=="2019/2020")) +
  geom_tile(aes(x=wk_date, y=regionnew, fill=z_score_bin, color=rili_minus_is_max), size=1, width=6.5, height=0.9) +
